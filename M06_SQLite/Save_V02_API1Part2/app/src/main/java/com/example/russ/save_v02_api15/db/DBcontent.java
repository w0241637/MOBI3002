...
//    public static Map<String, AnimalItem> ITEM_MAP = new HashMap<String, AnimalItem>();
    ...
    public DBcontent(Context context) {
        ...
        if (dbRaw.getCount() > 1) {
            ...
            AnimalItem a = new AnimalItem("1", "Djarra", "War has been your life for as long as you care to remember. You trained as a youth, studied the use of weapons and armor, learned basic survival techniques, including how to stay alive on the battlefield. You might have been part of a standing national army or a mercenary company, or perhaps a member of a local militia who rose to prominence during a recent war.", ACincrementer,lastTimeAccessed);
            dbRaw.addAnimal(a);

            a = new AnimalItem("2", "Kappa", "While you are unarmed or wielding only monk weapons and you aren’t wearing armor or wielding a shield, you can use DEX instead of STR for the attack and damage rolls, you can roll your Martial Arts damage die in place of the normal damage, and when you use the Attack action on your turn, you can make one unarmed strike as a bonus action.", ACincrementer,lastTimeAccessed);
            dbRaw.addAnimal(a);

            a = new AnimalItem("3", "Dro'Khar", "You're a giant humanoid cat rouge", ACincrementer,lastTimeAccessed);
            dbRaw.addAnimal(a);

            a = new AnimalItem("4", "Tumazara", "You ignore resistance to poison damage, can apply poison to a weapon or piece of ammunition as a bonus action, gain proficiency with poisoner's kit and can create a special poison.", ACincrementer,lastTimeAccessed);
            dbRaw.addAnimal(a);

            a = new AnimalItem("5", "Rio", "You are a half-ork who is second in command on the ship formally captained by Gwenevere Stormsword", ACincrementer,lastTimeAccessed);
            dbRaw.addAnimal(a);

            a = new AnimalItem("6", "Dungeon Mastea", "You're the world, and the referee", ACincrementer,lastTimeAccessed);
            dbRaw.addAnimal(a);
...
//        dbRaw.getCount();
    ...
    public class AnimalItem {
        ...
        public AnimalItem(String id, String name, String details, String access_count, String last_accessed) {
            this.id = id;
            this.name = name;
